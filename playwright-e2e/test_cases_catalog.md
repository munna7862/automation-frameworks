...
| ID | Title | Description |
|:---|:---|:---|
| **API_CART_01** | Persistence after server crash | Add item -> Call `/api/test/reset` (or restart server) -> Get Cart. Verify item is still there (T5). |
...
7.  **Verify**: Verify the order appears in the order history (if implemented) or that a Success ID is provided.
